...
cov.Prob.plot <- function(X,samples,tf,n=10,ymax=max(X)){
	plot(X/max(X),xlim=c(1,tf),ylim=c(0,ymax))
	Nsam=nrow(samples)
	...
	for(i in 1:n){
		...
		res=cov.sim(params,tf)
		pp=res[[1]]/(res[[1]]+params['k'])
	       	lines(pp,col=rgb(1,0,0,.5),lty=2)
	}
}
...
cov.ProbFromGlobal.plot <- function(ind,samples,tf,n=10){
	plot(data[ind,]/max(data[ind,]),xlim=c(1,tf),main=country[ind,])
	Nsam=length(samples)
	randsam=sample((Nsam/2):Nsam,n)
	for(i in 1:n){
		ranind=sample((Nsam/2):Nsam,1)
