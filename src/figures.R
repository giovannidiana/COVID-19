...
par_comb[[1]]=par_comb[[1]]=unlist(lapply(sam_glob4,"[[",'lambda'))
for(i in 2:4){
       	par_comb[[i]]=t(do.call(cbind,lapply(sam_glob4,function(x) x$param[,i-1])))
	colnames(par_comb[[i]])=labels
}

png("../Figures/Figure_stat_lambda.png",width=600,height=400)
hist(unlist(lapply(sam_glob4,function(x) x$lambda)),
     main="",
