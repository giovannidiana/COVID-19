...
cov.GlobalLogLikelihood <- function(lam,paramlist,Al,Bl,A,B,tf,temperature=1,print=F){
	...
	res=cov.SIM(p,tf)
	LogLik = dgamma(lam,Al,Bl,log=T)+sum(dpois(data,lambda=res[[1]],log=T)) + sum(dgamma(paramlist,matrix(A,nr,length(A),byrow=1),matrix(B,nr,length(B),byrow=1),log=T))
...
cov.GlobalLogLikelihood_vec <- function(lam,paramlist,Al,Bl,A,B,tf,temperature=1,print=F){
	...
	LogLikVec = rowSums(dpois(data,lambda=res[[1]],log=T)) + rowSums(dgamma(paramlist,matrix(A,nr,length(A),byrow=1),matrix(B,nr,length(B),byrow=1),log=T))
...
cov.GlobalMCMC <- function(NITER,id_show=1,adaptive=FALSE){
	...
	for(i in 1:NITER){
		...
		params_samples[[i]]=list(param=params,lambda=lambda)
			...
			#layout(matrix(1:4,2))
		       	#for(l in 1:4) 
		#		cov.plotFromGlobal(id_show,params_samples[(i-20):i],ncol(data)+10,10)
		#}
	}
