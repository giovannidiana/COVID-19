...
//Read the data
d3.csv("https://raw.githubusercontent.com/giovannidiana/COVID-19/gh-pages/src/AreaListCsv_sir_2503.csv", function(data) {

	var keys0 = d3.keys(data[0]).slice(1,)
	                           .map(function(d) {return d.slice(0,-4);});
	var NC = keys0.length;
	var keys=[];
	for ( var i=0;i<NC/8;i++){
	    keys.push(keys0[i]);
	}
	...
	// Add dots
		...
		scatter.append('g')
			   ...
			   .append("circle")
		          .attr("class","rec")
			      .attr("cx", function (d) { return x(d.t); } )
			      .attr("cy", function (d) { return y(d[currentGroup+'_REC']); } )
			      .attr("r", 2)
			      .style("fill","green")
  ...
  // add lines
		...
		scatter.append("path")
			   ...
			   .attr("d", d3.area()
							   ...
							   .y1(function(d) { return y(d[currentGroup+'_qr3']) })
							   );
